<?php
add_action('widgets_init','jellywp_ads200x760_load_widgets');

function jellywp_ads200x760_load_widgets(){
		register_widget("jellywp_ads200x760_widget");
}

class jellywp_ads200x760_widget extends WP_widget{

/*-----------------------------------------------------------------------------------*/
/*	Widget Setup
/*-----------------------------------------------------------------------------------*/

	function jellywp_ads200x760_widget(){
		$widget_ops = array( 'classname' => 'jellywp_ads728x90_widget', 'description' => esc_attr__( 'Ads 200x760' , 'jelly_text_main') );
		parent::__construct('jellywp_ads728x90_widget', esc_attr__('jellywp: Ads 200x760', 'jelly_text_main'), $widget_ops);
	}
		...
		<div class="widget">
			<div class="ads728x90-thumb">
				<a href="<?php if($link != ""){echo esc_attr($link);}else{echo esc_attr("#");} ?>">
					<img src="<?php if($image != ""){echo esc_attr($image);}
					else{echo get_template_directory_uri()."/img/200x760.jpg";} ?>" alt="" />
				</a>
			</div> 	
		</div> 
		<?php
